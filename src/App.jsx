...
const HELP_SECTIONS = [
  ...
  },
  {
    ...
    body: "Only the story starter (Premium) can lock or unlock. Locked stories stay on the timeline with a padlock. When locked, Story settings show the full invite code, a copy button (shows a short Copied message), and one padlock on the right — the padlock does not cover the code. Story invites expire at The End; community members keep access and can see the finished story.",
  },
  {
    title: "Colors & themes",
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Lock: full invite code + copy; padlock on the right only — never covers letters */}
        <div
          style={{
            marginBottom: 20,
            padding: 14,
              ...
              <div style={{ fontSize: 13, fontWeight: 600, color: "var(--ink)", marginBottom: 4 }}>
                {chain.locked ? (
                  <span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
                    <Lock size={14} /> Locked (on timeline)
                  </span>
                ) : (
                  <span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
                    <Unlock size={14} /> Open story
                  </span>
                )}
              </div>
              {chain.locked && chain.inviteCode && !chain.finished && !chain.inviteExpired && (
                <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", marginTop: 8 }}>
                  <span style={{ fontSize: 14, color: "var(--ink)", wordBreak: "break-all" }}>
                    Invite code:{" "}
                    <strong style={{ fontFamily: "ui-monospace, monospace", letterSpacing: "0.06em" }}>
                      {chain.inviteCode}
                    </strong>
                  </span>
                  <button
                  ...
                  {copiedInvite && (
                    <span style={{ fontSize: 12, color: "#1D9BF0", fontWeight: 600 }}>Copied</span>
                  )}
                </div>
              )}
              {chain.locked && (chain.finished || chain.inviteExpired) && (
                <p style={{ fontSize: 12, color: "var(--ink-soft)", margin: "8px 0 0 0" }}>
                  Story invite expired when this story ended. Community members still have access.
                </p>
              )}
              {!canLock && (
                <p style={{ fontSize: 12, color: "var(--ink-soft)", margin: "4px 0 0 0" }}>Premium subscribers</p>
              )}
            </div>
            {canLock && (
              <button
                ...
                title={chain.locked ? "Unlock story" : "Lock (invite only)"}
