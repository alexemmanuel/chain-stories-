...
// Multi-user cloud (reads from .env)
...
/** Anti-spam: sustained rapid actions → progressive suspension (local device). */
const SPAM_WINDOW_MS = 60 * 1000;
const SPAM_ACTION_THRESHOLD = 45; // actions in 60s window
...
const SPAM_WARN_MS = 30 * 1000; // one-time red warning duration
const SPAM_STORAGE_KEY = "chn-spam-guard-v1";
...
/** Shift lightness of a hex while keeping hue proportions (same-family surfaces). */
function shiftLightness(hex, delta) {
  const [r, g, b] = parseHexRgb(hex);
  return toHexRgb(r + delta * 255, g + delta * 255, b + delta * 255);
}
...
function avoidPureMono(hex, paper) {
  ...
  if (L > 0.92) return mixHex(hex, base, 0.42);
  if (L < 0.08) return mixHex(hex, base, 0.42);
  return hex;
}
...
/**
 * Build a full UI theme from any base paper color so the entire app
 * (lobby, chain room, settings, inputs, buttons) immerses in that hue —
 * same family as the screenshots: neon green / blue fill the whole interface.
 * Only white & black themes may use pure B/W chrome; sand/custom never do.
 * Green is allowed for text whenever contrast is solid.
 */
function themeFromCustom(hex) {
  ...
  // Same-hue surfaces (like the green/blue examples): subtle lightness shift only
  let paper2 = avoidPureMono(shiftLightness(paper, dark ? 0.08 : -0.06), paper);
  let card = avoidPureMono(shiftLightness(paper, dark ? 0.12 : -0.04), paper);
  if (Math.abs(luminance(paper2) - L) < 0.03) {
    paper2 = avoidPureMono(mixHex(paper, dark ? "#FFFFFF" : "#1A1510", dark ? 0.1 : 0.1), paper);
  }
  if (Math.abs(luminance(card) - L) < 0.03) {
    card = avoidPureMono(mixHex(paper, dark ? "#FFFFFF" : "#1A1510", dark ? 0.14 : 0.12), paper);
  }

  // Text: prefer green when it reads well; otherwise warm / cool / neutral
  ...
  let ink = avoidPureMono(mixHex(seed, paper, 0.12), paper);
  // Ensure readable contrast; green is the preferred rescue color
  if (Math.abs(luminance(ink) - L) < 0.28) {
    const greenRescue = dark ? "#86EFAC" : "#166534";
    ink = avoidPureMono(mixHex(greenRescue, paper, 0.15), paper);
    if (Math.abs(luminance(ink) - L) < 0.28) {
      ink = avoidPureMono(mixHex(dark ? "#F5F0FF" : "#1A1020", paper, 0.08), paper);
    }
  }
  const inkSoft = avoidPureMono(mixHex(ink, paper, 0.45), paper);

  const [ir, ig, ib] = parseHexRgb(ink);
  const border = `rgba(${ir},${ig},${ib},0.22)`;

  // Buttons: muted same-family (olive on green, lavender on blue) — never pure B/W
  let btn = avoidPureMono(
    dark ? mixHex(paper, "#E9D5FF", 0.35) : mixHex(paper, "#1A1510", 0.28),
    paper
  );
  if (Math.abs(luminance(btn) - L) < 0.18) {
    btn = avoidPureMono(
      dark ? mixHex(paper, "#FFFFFF", 0.28) : mixHex(paper, "#0A0A0A", 0.35),
      paper
    );
  }
  const btnInk = luminance(btn) < 0.5
    ? avoidPureMono(mixHex("#F5F0FF", paper, 0.1), paper)
    : avoidPureMono(mixHex("#1A1020", paper, 0.12), paper);

  const btnSecondary = avoidPureMono(mixHex(paper, ink, 0.18), paper);
  const btnSecondaryInk = ink;

  // Danger: red family; on red paper use high-contrast non-red pair
  const paperIsRed = pr0 > pg0 + 40 && pr0 > pb0 + 40;
  let danger;
  let dangerInk;
  if (paperIsRed) {
    danger = avoidPureMono(mixHex("#1A1020", paper, 0.15), paper);
    dangerInk = avoidPureMono(mixHex("#F5F0FF", paper, 0.1), paper);
  } else {
    danger = avoidPureMono(mixHex("#DC2626", paper, 0.1), paper);
    dangerInk = avoidPureMono(mixHex("#FFF1F2", paper, 0.15), paper);
    if (Math.abs(luminance(danger) - L) < 0.25) {
      danger = avoidPureMono(mixHex("#9F1239", paper, 0.2), paper);
    }
  }
...
/**
 * Theme resolution:
 * - Inside a story: if the Premium starter set a story theme (bgCustom / bgThemeId),
 *   that theme paints the whole story UI for every writer (free or Premium).
 * - Otherwise (and everywhere outside stories): personal profile theme.
 * - Free users only get White / Black / Sand as personal themes.
 * - Only the Premium story starter may set or change story themes.
 */
function resolveTheme(profile, chain) {
  if (chain?.bgCustom) {
    return themeFromCustom(chain.bgCustom);
  }
  if (chain?.bgThemeId) {
    const storyBase = FREE_BG_THEMES.find((t) => t.id === chain.bgThemeId);
    if (storyBase) return enrichTheme(storyBase);
  }
...
function loadSpamGuard() {
  ...
  try {
    ...
    return {
      ...
      actionLog: Array.isArray(d.actionLog) ? d.actionLog : [],
...
/**
 * Record a user action for spam detection.
 * Returns { ok, suspended, remainingMs, throttled, warning }.
 * warning = one-time lifetime red alert on the last steps before first suspension.
 */
function recordSpamAction(kind) {
  const now = Date.now();
  const s = loadSpamGuard();

  // Clear expired suspension; mark post-ban watch so next spam → 24h
  ...
  // Prune log to window
  s.actionLog = (s.actionLog || []).filter((t) => now - t < SPAM_WINDOW_MS);
  s.actionLog.push(now);

  if (s.actionLog.length >= SPAM_ACTION_THRESHOLD) {
    const until = applySpamGuilt(kind || "burst");
    return {
      ok: false,
      suspended: true,
  ...
  // One lifetime warning on the last two actions before first suspension threshold
  let warning = false;
  if (
    !s.warnedLifetime &&
    (s.guiltCount || 0) === 0 &&
    s.actionLog.length >= Math.max(1, SPAM_ACTION_THRESHOLD - 2)
  ) {
  ...
  saveSpamGuard(s);
  return { ok: true, suspended: false, remainingMs: 0, throttled: false, warning };
}
...
const HELP_SECTIONS = [
  ...
  },
  {
    ...
    body: "Free: write on open stories, join communities with a code, like stories, use White/Black/Sand personal themes (applies app-wide), ambient music, Watch mode, and vote on multi-writer actions. Premium adds: lock/unlock stories and invite codes, create communities, custom personal theme colors (hex/picker), story themes in Story settings (Premium starter only — paints that story for every writer), offline HTML reel download, and the CS badge on boards. If the host is Free, the story stays on each writer's personal theme. Premium is a demo toggle on the Board — no real payment in this build.",
  },
  {
    title: "Communities",
    keys: ["community", "communities", "members", "dissolve", "join community", "create community"],
    body: "Only Premium subscribers can create a community. Anyone (Free or Premium) can join with a community code. Members get direct access to each other's stories (including locked) without a story invite and can see each other's names. The community starter can remove a member — that person needs story invites again for locked stories. Under 20 active members after 30 days the community dissolves permanently.",
  },
  {
    title: "Admins",
  ...
  },
  {
    ...
    body: "Rapid repeated taps are throttled so the app stays smooth. Pop-up toasts that fade (such as limit flashes) stop reappearing after two shows for about 20 seconds while the underlying action still works. Sustained spam for about a minute can suspend this device: first suspension starts at 30 minutes and grows by 15 minutes with further guilt; five guilt strikes become a 24-hour suspension. After any ban ends, one more spam episode brings an immediate 24-hour suspension. Before the first suspension only, a single lifetime red warning may appear for 30 seconds: “Stop spamming! Continuous behaviour will attract a time based ban.” While suspended, the app shows a scrolling notice with the time left until access returns.",
  },
];
...
function ChainRoom({ profile, chainId, onBack, onUpdateProfile, onOpenHelp }) {
    ...
    const next = idx.map((c) =>
      c.id === updated.id
        ? {
            ...
            lastLine: updated.lines[updated.lines.length - 1]?.text || "",
            lastAuthor: updated.lines[updated.lines.length - 1]?.authorName || "",
  ...
  const castVote = async (type, choice) => {
    // Device spam / suspension check (all users)
    const spam = recordSpamAction(`vote:${type}:${choice}`);
    if (spam.suspended) return "suspended";
  ...
  /**
   * Story theme — Premium story starter only.
   * Free hosts cannot set a story theme (writers keep their personal themes).
   * Non-starters (even Premium) cannot change another writer's story theme.
   */
  const setStoryBg = async (themeId, customHex) => {
    ...
    const clear = themeId == null && customHex == null;
    const updated = {
      ...latest,
      bgThemeId: clear ? null : safeCustom ? null : themeId || null,
      bgCustom: clear ? null : safeCustom || null,
      updatedAt: Date.now(),
    };
    await setJSON(`chain:${chainId}`, updated, true);
    setChain(updated);
...
function StorySettingsModal({
  ...
  const wrapStart = (fn) => async () => {
    const result = await fn?.();
    if (result === "limit") flashVoteLimit();
    // throttle / cooldown / suspended: silent — no extra UI spam
  };
  const canLock = !!profile?.premium && isStoryStarter(chain, profile.id);
  const canSetStoryTheme = !!profile?.premium && isStoryStarter(chain, profile.id);
  const [titleDraft, setTitleDraft] = useState(chain.title || "");
  const [titleMsg, setTitleMsg] = useState("");
  const [storyHexDraft, setStoryHexDraft] = useState(chain.bgCustom || "#1D9BF0");
  const admins = ensureAdmins(chain);
  const changesUsed = titleChangesInWindow(chain);
  const titleEditable = canEditTitle(chain);
  const activeStoryThemeId = chain.bgCustom ? "custom" : chain.bgThemeId || null;
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Lock control: padlock only — no “Unlock story” / “Locked (on timeline)” labels */}
        <div
          style={{
            position: "relative",
            marginBottom: 20,
        ...
        >
          {chain.locked && chain.inviteCode && !chain.finished && !chain.inviteExpired && (
            <div
              style={{
                ...
                flex: "1 1 auto",
                minWidth: 0,
                paddingRight: canLock ? 52 : 0,
              }}
            >
              <p style={{ fontSize: 13, margin: 0, color: "var(--ink)", wordBreak: "break-all" }}>
                Invite code: <strong>{chain.inviteCode}</strong>
              </p>
              <button
                type="button"
                onClick={() => {
                  try {
                    navigator.clipboard?.writeText(chain.inviteCode);
                  } catch (_) {}
                }}
          ...
          {chain.locked && (chain.finished || chain.inviteExpired) && (
            <p style={{ fontSize: 12, color: "var(--ink-soft)", margin: 0, flex: 1, paddingRight: canLock ? 52 : 0 }}>
              Story invite expired when this story ended. Community members still have access.
            </p>
          )}
          {!chain.locked && !canLock && (
            <p style={{ fontSize: 12, color: "var(--ink-soft)", margin: 0 }}>Premium subscribers</p>
          )}
          {canLock && (
            <button
              type="button"
              ...
              style={{
                position: "absolute",
                top: chain.locked && !(chain.inviteCode && !chain.finished && !chain.inviteExpired)
                  ? "50%"
                  : 10,
                right: chain.locked && !(chain.inviteCode && !chain.finished && !chain.inviteExpired)
                  ? "50%"
                  : 10,
                transform:
                  chain.locked && !(chain.inviteCode && !chain.finished && !chain.inviteExpired)
                    ? "translate(50%, -50%)"
                    : "none",
                transition: "top 0.28s ease, right 0.28s ease, transform 0.28s ease",
                width: 44,
                height: 44,
                borderRadius: 999,
                border: "1px solid var(--border)",
        ...
        {/* Story theme — Premium starter only; paints the story for every writer */}
        {canSetStoryTheme && (
          <div
            style={{
              marginBottom: 20,
          ...
          >
            <div style={{ fontSize: 14, fontWeight: 700, marginBottom: 6, color: "var(--ink)" }}>
              Story theme
            </div>
            <p style={{ fontSize: 12, color: "var(--ink-soft)", margin: "0 0 12px 0", lineHeight: 1.45 }}>
              Applies for everyone writing on this story. Your personal theme still applies outside it.
            </p>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 10, alignItems: "center" }}>
              {FREE_BG_THEMES.map((t) => {
                const selected = activeStoryThemeId === t.id;
                return (
                  <button
                    key={t.id}
                    type="button"
                    onClick={() => onSetStoryBg?.(t.id, null)}
                    title={t.label}
                    aria-label={t.label}
                    style={{
                      width: 36,
                      ...
                      background: t.paper,
                      cursor: "pointer",
                      boxShadow: selected ? "0 0 0 2px rgba(29,155,240,0.25)" : "none",
                    }}
                  />
                );
              })}
              <label
                style={{
                  ...
                  height: 36,
                  borderRadius: 10,
                  border: activeStoryThemeId === "custom" ? "2px solid #1D9BF0" : "1px solid var(--border)",
                  background: chain.bgCustom || "var(--paper-card)",
                  cursor: "pointer",
                  display: "flex",
                  alignItems: "center",
                  justifyContent: "center",
              ...
              >
                ...
                <input
                  type="color"
                  value={/^#[0-9A-Fa-f]{6}$/.test(storyHexDraft || "") ? storyHexDraft : "#1D9BF0"}
                  onChange={(e) => {
              ...
              {activeStoryThemeId && (
                <button
                  type="button"
                  onClick={() => onSetStoryBg?.(null, null)}
                  className="chn-btn"
                  style={{
                    padding: "6px 12px",
                    borderRadius: 999,
...
export default function App() {
  ...
  // Suspension countdown + light global spam sampling on rapid clicks
  useEffect(() => {
    ...
    const onPointer = () => {
      const now = Date.now();
      if (now - lastSample < 80) return; // sample at most ~12/s
      lastSample = now;
      const r = recordSpamAction("pointer");
      if (r.suspended) setSuspendLeft(r.remainingMs);
      if (r.warning) {
        setSpamWarn(true);
        if (spamWarnTimer.current) clearTimeout(spamWarnTimer.current);
        spamWarnTimer.current = setTimeout(() => setSpamWarn(false), SPAM_WARN_MS);
      }
    };
    ...
    return (
      ...
      >
        ...
        <p style={{ fontSize: 14, color: "var(--ink-soft)", textAlign: "center", maxWidth: 320, lineHeight: 1.5, margin: 0 }}>
          This device is paused for excessive rapid activity.
        </p>
      </div>
    );
  }
