...
/** Auto theme: alternate personal ↔ story theme every 30 minutes while in a chain. */
...
/** Personal theme from the writer profile. */
...
/**
 * Story theme stored on the chain (set from starter at create; starter can refresh later).
 * Falls back to personal if the chain has no storyTheme yet.
 */
function resolveStoryTheme(chain, profileFallback) {
  const st = chain?.storyTheme;
  if (st?.themeId === "custom" && st?.themeCustom) {
    return themeFromCustom(st.themeCustom);
  }
  if (st?.themeId) {
 ...
 * Active UI theme.
 * mode: "mine" | "auto"
 * useStory: when mode is auto, true = story half of the cycle
 */
function resolveTheme(profile, chain, opts = {}) {
  const mode = opts.mode || "mine";
  const useStory = !!opts.useStory;
  if (mode === "auto" && useStory && chain) {
    return resolveStoryTheme(chain, profile);
  }
...
function themePrefKey(profileId) {
  return `chn-theme-pref:${profileId || "anon"}`;
}
function themeAutoKey(chainId, profileId) {
  return `chn-theme-auto:${chainId}:${profileId || "anon"}`;
}

function loadThemeMode(profileId) {
  try {
    const v = localStorage.getItem(themePrefKey(profileId));
    return v === "auto" ? "auto" : "mine";
  } catch (_) {
    return "mine";
  }
}
...
function saveThemeMode(profileId, mode) {
  try {
    localStorage.setItem(themePrefKey(profileId), mode === "auto" ? "auto" : "mine");
  } catch (_) {}
}
...
function loadAutoState(chainId, profileId) {
  try {
    ...
    return { startedAt: Date.now(), useStory: false };
  }
}

function saveAutoState(chainId, profileId, state) {
  try {
    localStorage.setItem(themeAutoKey(chainId, profileId), JSON.stringify(state));
  } catch (_) {}
}
...
function softApplyTheme(theme) {
  ...
  // clear transition after fade so instant changes (lobby) stay snappy
  setTimeout(() => {
...
const HELP_SECTIONS = [
  ...
  {
    title: "Colors & themes",
    keys: ["theme", "color", "auto theme", "story theme", "my theme"],
    body: "Your personal theme is set in profile. Inside a chain: My theme keeps your colors; Auto alternates your theme and the story theme every 30 minutes with a soft fade. Story theme is stored on the chain (starter's colors at create, or Set story theme). Auto will not hard-switch while you are typing a line — it waits until you leave the box.",
  },
    title: "Locks & invites",
...
function Lobby({ profile, onOpenChain, onUpdateProfile, onOpenArchives, onOpenHelp }) {
  ...
  const createChain = async ({ title, genre, theme, solo, target }) => {
    ...
    const chain = {
      ...
      likes: 0,
      watches: 0,
      // Story theme = starter's theme at create (stored on chain for Auto mode)
...
function ChainRoom({ profile, chainId, onBack, onUpdateProfile, onOpenHelp }) {
  ...
  // Theme mode: mine | auto (30 min interchange personal ↔ story theme)
  ...
  const [themeLeftMs, setThemeLeftMs] = useState(THEME_AUTO_MS);
  ...
  const activeTheme = resolveTheme(profile, chain, {
    mode: themeMode,
    useStory: themeMode === "auto" && autoState.useStory,
  });

  useEffect(() => {
    softApplyTheme(activeTheme);
  }, [profile?.themeId, profile?.themeCustom, chain?.storyTheme?.themeId, chain?.storyTheme?.themeCustom, themeMode, autoState.useStory]);

  // Auto countdown + switch (skip hard switch while typing — queue hint, apply on blur)
  useEffect(() => {
    if (themeMode !== "auto" || !chain) {
      setQuietThemeHint("");
    ...
    const tick = () => {
      ...
      setThemeLeftMs(left);
      // crossed a boundary
      const periods = Math.floor(elapsed / THEME_AUTO_MS);
      const shouldStory = periods % 2 === 1;
      if (shouldStory !== st.useStory) {
        if (typingFocused) {
          setPendingThemeSwitch(true);
          setQuietThemeHint(
            shouldStory
              ? "Story theme ready — applies when you leave the line box"
              : "Your theme ready — applies when you leave the line box"
          );
        } else {
          ...
          setPendingThemeSwitch(false);
          setQuietThemeHint(
            shouldStory ? "Switched to story theme" : "Switched to your theme"
          );
        ...
        setQuietThemeHint(
          st.useStory
            ? `Story theme · your theme in ~${mins}m`
            : `Your theme · story theme in ~${mins}m`
        );
      }
    };
    tick();
  ...
  // Apply pending switch when writer leaves the text box
  useEffect(() => {
    ...
    <div className="chn-root" style={{ minHeight: "100vh" }}>
      ...
      <div className="chn-shell">
        ...
        >
          ...
          {/* Theme: My colors | Auto (30 min ↔ story theme) */}
          <div
            style={{
              display: "flex",
              flexWrap: "wrap",
          ...
          >
            <span style={{ fontSize: 11, fontWeight: 600, color: "var(--ink-soft)", letterSpacing: "0.04em" }}>
              THEME
            </span>
            <button
              type="button"
              onClick={() => setThemeModeAndSave("mine")}
              style={{
                fontSize: 12,
                padding: "6px 12px",
                borderRadius: 999,
                ...
                color: themeMode === "mine" ? "var(--btn-ink)" : "var(--ink)",
                cursor: "pointer",
                fontWeight: 600,
              }}
            >
            ...
            <button
              type="button"
              onClick={() => setThemeModeAndSave("auto")}
              style={{
                fontSize: 12,
                padding: "6px 12px",
                borderRadius: 999,
                ...
                color: themeMode === "auto" ? "var(--btn-ink)" : "var(--ink)",
                cursor: "pointer",
                fontWeight: 600,
              }}
            >
            ...
            {isStoryStarter(chain, profile.id) && (
              <button
                ...
                title="Store your current colors as this story's theme"
              >
                Set story theme
              </button>
            )}
          ...
          {quietThemeHint && (
            <p
              style={{
                fontSize: 12,
                color: pendingThemeSwitch ? "var(--danger)" : "var(--ink-soft)",
                margin: "6px 0 0 0",
                lineHeight: 1.4,
              }}
            >
