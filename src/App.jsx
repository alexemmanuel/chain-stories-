...
import {
  ...
  Volume2, VolumeX, ExternalLink, Music
} from "lucide-react";
...
function themeFromCustom(hex) {
  ...
  const paper2 = avoidPureMono(
    dark ? mixHex(paper, "#0A0A0A", 0.28) : mixHex(paper, "#1A1510", 0.08),
    paper
  );
  const card = avoidPureMono(
    dark ? mixHex(paper, "#E8E0F0", 0.12) : mixHex(paper, "#FFF8F0", 0.4),
    paper
  );
...
function CommunitiesModal({ profile, onClose, onUpdateProfile }) {
  ...
  const dissolveCommunity = async (communityId) => {
    ...
    if (!full || full.createdById !== profile.id || full.dissolved) return;
    setBusy(true);
    const updated = {
      ...full,
      dissolved: true,
      dissolvedAt: Date.now(),
      dissolvedBy: profile.id,
    };
    await saveCommunity(updated);
    if (selected?.id === id) setSelected(null);
    setConfirmDeleteId(null);
    setBusy(false);
    setMsg(`“${full.name}” deleted.`);
    reload();
  };
      ...
      >
        ...
        {selected && !selected.dissolved && (
          <div style={{ marginTop: 16, padding: 12, borderRadius: 12, background: "var(--paper-2)" }}>
            ...
            >
              <div style={{ fontWeight: 700 }}>{selected.name} · members</div>
              {selected.createdById === profile.id && confirmDeleteId !== selected.id && (
                <button
                  type="button"
                  onClick={() => {
            ...
            {confirmDeleteId === selected.id && (
              <div style={{ marginBottom: 12 }}>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  <button
                    type="button"
                    onClick={() => dissolveCommunity(selected.id)}
                    disabled={busy}
                    className="chn-btn chn-btn-danger"
                    style={{
                      flex: 1,
        ...
        >
          {canLock ? (
            <button
              ...
              style={{
                position: "absolute",
                // Locked → closed padlock centered; unlocked → open padlock top-right
                top: chain.locked ? "50%" : 10,
                right: chain.locked ? "50%" : 10,
                transform: chain.locked ? "translate(50%, -50%)" : "none",
          ...
          {chain.locked && chain.inviteCode && !chain.finished && !chain.inviteExpired && (
            <p style={{ fontSize: 13, margin: "8px 48px 0 0", color: "var(--ink)" }}>
              Invite code: <strong>{chain.inviteCode}</strong>
            </p>
          )}
          {chain.locked && (chain.finished || chain.inviteExpired) && (
            <p style={{ fontSize: 12, color: "var(--ink-soft)", margin: "8px 48px 0 0" }}>
