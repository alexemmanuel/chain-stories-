...
const THEME_AUTO_MS = 30 * 60 * 1000;
...
function resolvePersonalTheme(profile) {
...
function resolveStoryTheme(chain, profileFallback) {
  const st = chain?.storyTheme;
  if (st?.themeId === "custom" && st?.themeCustom) return themeFromCustom(st.themeCustom);
  if (st?.themeId) {
    const base = FREE_BG_THEMES.find((t) => t.id === st.themeId) || FREE_BG_THEMES[0];
    return enrichTheme(base);
  }
  return resolvePersonalTheme(profileFallback);
}
function resolveTheme(profile, chain, opts = {}) {
  if ((opts.mode || "mine") === "auto" && opts.useStory && chain) return resolveStoryTheme(chain, profile);
  return resolvePersonalTheme(profile);
}
function storyThemeFromProfile(profile) {
  return {
    themeId: profile?.themeId || "white",
    themeCustom: profile?.themeId === "custom" ? profile?.themeCustom || null : null,
    setById: profile?.id || null,
    setAt: Date.now(),
  };
}
function themePrefKey(profileId) { return `chn-theme-pref:${profileId || "anon"}`; }
function themeAutoKey(chainId, profileId) { return `chn-theme-auto:${chainId}:${profileId || "anon"}`; }
function loadThemeMode(profileId) {
  try { return localStorage.getItem(themePrefKey(profileId)) === "auto" ? "auto" : "mine"; }
  catch (_) { return "mine"; }
}
function saveThemeMode(profileId, mode) {
  try { localStorage.setItem(themePrefKey(profileId), mode === "auto" ? "auto" : "mine"); } catch (_) {}
}
function loadAutoState(chainId, profileId) {
  try {
    const raw = localStorage.getItem(themeAutoKey(chainId, profileId));
    if (!raw) return { startedAt: Date.now(), useStory: false };
    const o = JSON.parse(raw);
    return { startedAt: typeof o.startedAt === "number" ? o.startedAt : Date.now(), useStory: !!o.useStory };
  } catch (_) { return { startedAt: Date.now(), useStory: false }; }
}
function saveAutoState(chainId, profileId, state) {
  try { localStorage.setItem(themeAutoKey(chainId, profileId), JSON.stringify(state)); } catch (_) {}
}
function softApplyTheme(theme) {
  if (typeof document === "undefined") return;
  const root = document.documentElement;
  root.style.transition = "background-color 0.7s ease, color 0.7s ease";
  if (document.body) document.body.style.transition = "background-color 0.7s ease, color 0.7s ease";
...
// ---------- storage ----------
// shared=true  → Supabase (visible to all users of this app)
// shared=false → localStorage (this device only: profile, spam, theme prefs)
//
// Setup:
// 1) Create a free project at https://supabase.com
// 2) SQL Editor → run the schema in SUPABASE_SCHEMA (see comment below)
// 3) Project Settings → API → copy URL + anon key into .env:
//      VITE_SUPABASE_URL=https://xxxx.supabase.co
//      VITE_SUPABASE_ANON_KEY=eyJ...
// 4) npm i @supabase/supabase-js && redeploy
//
// Without env vars the app still runs (local-only, same as before).

const SUPABASE_URL = (typeof import.meta !== "undefined" && import.meta.env && import.meta.env.VITE_SUPABASE_URL) || "";
const SUPABASE_ANON = (typeof import.meta !== "undefined" && import.meta.env && import.meta.env.VITE_SUPABASE_ANON_KEY) || "";
...
async function getSupabase() {
  ...
  if (!SUPABASE_URL || !SUPABASE_ANON) {
    console.info("[chain-stories] Supabase not configured — using localStorage only. Set VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY to sync across users.");
    return null;
  }
  try {
    ...
    console.warn("[chain-stories] @supabase/supabase-js missing — run: npm i @supabase/supabase-js", e);
    return null;
  }
}

function isSharedKey(key, shared) {
  if (shared === false) return false;
  if (shared === true) return true;
  // Heuristic if caller omits flag
  return (
    key === "chains-index" ||
...
const storage = {
    ...
    try {
      if (isSharedKey(key, shared)) {
        ...
        if (sb) {
          ...
          } else if (data != null && data.value != null) {
            const raw = typeof data.value === "string" ? data.value : JSON.stringify(data.value);
            try {
              localStorage.setItem(`chn-cache:${key}`, raw);
            } catch (_) {}
            return { value: raw };
          } else {
            return null;
      ...
      const value = localStorage.getItem(key) ?? localStorage.getItem(`chn-cache:${key}`);
  ...
  async set(key, value, shared) {
    // value is already a JSON string from setJSON
    if (isSharedKey(key, shared)) {
      const sb = await getSupabase();
      if (sb) {
        let parsed;
        ...
        try {
          localStorage.setItem(`chn-cache:${key}`, value);
        } catch (_) {}
        return;
      }
    }
  ...
  async delete(key, shared) {
    if (isSharedKey(key, shared)) {
      const sb = await getSupabase();
      if (sb) {
        const { error } = await sb.from("app_store").delete().eq("key", key);
        if (error) console.warn("[storage.delete]", key, error.message);
        try {
          localStorage.removeItem(`chn-cache:${key}`);
        } catch (_) {}
        return;
      }
    }
...
/**
 * Optional: one-time push of local shared keys to Supabase so an existing
 * writer does not lose their chains when enabling cloud sync.
 */
async function migrateLocalSharedToSupabase() {
  const sb = await getSupabase();
  if (!sb) return { ok: false, reason: "no-supabase" };
  const flag = "chn-migrated-to-supabase-v1";
  try {
    if (localStorage.getItem(flag)) return { ok: true, skipped: true };
  } catch (_) {}
  const keys = [];
  try {
    for (let i = 0; i < localStorage.length; i++) {
      ...
      if (!k) continue;
      if (k === "chains-index" || k === "communities-index" || k.startsWith("chain:") || k.startsWith("community:")) {
        keys.push(k);
      }
    }
  } catch (_) {}
  ...
  for (const k of keys) {
    try {
      ...
      // Do not overwrite newer cloud data
      const { data: existing } = await sb.from("app_store").select("key").eq("key", k).maybeSingle();
      if (existing) continue;
      const { error } = await sb.from("app_store").upsert(
        { key: k, value: parsed, updated_at: new Date().toISOString() },
        { onConflict: "key" }
      );
      if (!error) n += 1;
    } catch (_) {}
...
-- Public read/write with anon key (demo app). Tighten later with auth + RLS.
create policy "anon read app_store"
  on public.app_store for select
  to anon, authenticated
  using (true);

create policy "anon write app_store"
  on public.app_store for insert
  to anon, authenticated
  with check (true);

create policy "anon update app_store"
  on public.app_store for update
  to anon, authenticated
  using (true)
  with check (true);

create policy "anon delete app_store"
  on public.app_store for delete
  to anon, authenticated
  using (true);

-- Optional realtime (Lobby live refresh):
-- Database → Replication → enable app_store
*/
...
const HELP_SECTIONS = [
  ...
  {
    title: "Titles",
    keys: ["title", "rename", "ai suggest", "revoke", "3/3", "save title"],
    body: "Anyone can edit a title before conversion, up to 3 times per 24 hours. Typing in the title box does not use a change. AI suggest only fills the box — you must tap Save to apply. Only a successful Save of a different title counts as 1 of the 3 daily changes. Saving the same title again uses none. Changes can be revoked by a writer majority vote. After conversion the title is permanent. Timeline cards update when the title is saved.",
  },
  {
    title: "Locks & invites",
    keys: ["lock", "unlock", "invite", "padlock", "code", "premium lock", "copy", "copied"],
    body: "Only the story starter (Premium) can lock or unlock. Locked stories stay on the timeline with a padlock. When locked, Story settings show only the invite code, a copy button (short Copied message), and one padlock — no extra labels. Story invites expire at The End; community members keep access and can see the finished story.",
  },
  {
    title: "Colors & themes",
    keys: ["theme", "color", "auto theme", "story theme", "my theme", "sand", "custom", "30 min", "auto"],
    body: "Set your personal theme in profile (White, Black, Sand, or Premium custom hex). Inside a chain: My theme keeps your colors; Auto alternates your theme and the story theme every 30 minutes with a soft fade. Story theme is stored on the chain from the starter's colors when the chain was created. Auto will not hard-switch while you are typing a line — it waits until you leave the box and shows a quiet hint (for example story theme in ~30m). Lobby always uses your personal theme.",
  },
  {
    title: "Premium vs Free",
    keys: ["premium", "free", "subscribe", "subscription", "benefit", "plan", "upgrade"],
    body: "Free: write on open stories, join communities with a code, like stories, use White/Black/Sand themes, ambient music, Watch mode, and vote on multi-writer actions. Premium adds: lock/unlock stories and invite codes, create communities (max 2 per user), custom theme colors (hex/picker), offline HTML reel download, and the CS badge on boards. Premium is a demo toggle on the Board — no real payment in this build.",
  },
  {
    title: "Communities",
    keys: ["community", "communities", "members", "dissolve", "join community", "create community", "delete community", "trash"],
    body: "Only Premium can create a community, and each user can create at most 2. Anyone (Free or Premium) can join with a community code. Members get direct access to each other's stories (including locked) without a story invite and can see each other's names. The community starter can delete the community from the list row (trash icon, with Yes/No confirm) — not from the members panel. The starter can also remove a member; that person needs story invites again for locked stories. Under 20 active members after 30 days the community dissolves permanently.",
  },
  {
    title: "Admins",
    keys: ["admin", "election", "kick", "nominate", "portal"],
    body: "Max 5 admins; the starter is always the first. Writers vote to elect more admins. The portal closes after about 50% of writers have voted and can reopen when the cast grows by about 50% again. Admins can start kick votes. A confidence vote can strip the starter's admin seat only — the starter stays in the story.",
  },
  {
    title: "Votes & archives",
    keys: ["vote", "archive", "unarchive", "delete", "majority", "petition"],
  ...
  },
  {
    ...
    keys: ["supabase", "cloud", "sync", "other users", "share", "online"],
    body: "When Supabase is configured, chains and communities are stored in the cloud so other people can see and join the same stories on the shared timeline. Your writer profile and spam state stay on this device. Without Supabase keys the app still works on one device only.",
  },
  {
    title: "Guide vs Help",
    keys: ["guide", "help", "chat", "assistant"],
    body: "Guide is the chat assistant for quick questions — it reads these Help rules and answers from them. Help (this screen) is the full rules reference. Ask Guide about titles, locks, copy invite code, communities, themes, auto theme, or spam rules.",
  },
  {
    title: "Spam protection & suspension",
    keys: ["spam", "suspend", "ban", "throttle", "warning", "stop spamming", "denied"],
...
const BOT_EXTRA = [
  ...
  {
    keys: ["auto theme", "my theme", "story theme", "30 minutes", "theme switch"],
    answer: "In a chain use My theme (your colors) or Auto (soft-fades between your theme and the story theme every 30 minutes; the hint shows the countdown). Story theme is saved when the starter creates the chain. Auto waits if you are typing a line.",
  },
  {
    keys: ["copy invite", "copy code", "copied"],
    answer: "When a story is locked, Story settings show the full invite code, a copy button, and one padlock on the right. Tap copy for a short Copied message. The padlock does not cover the code.",
  },
  {
    keys: ["delete community", "trash community"],
    answer: "Only the community starter can delete it. Use the trash icon on the community list row (not the members panel). Confirm Yes or No. Each user may create at most 2 communities.",
  },
  {
    keys: ["title change", "title save", "3 titles"],
    answer: "You get 3 title changes per 24 hours. Typing or AI suggest does not count until you Save a different title. Saving the same title uses no change.",
  },
...
/** Password: 8+ chars, upper, lower, number, symbol; no spaces or hyphens. */
function validatePassword(pw) {
  const s = String(pw || "");
  if (/\s/.test(s)) return "Password cannot contain spaces.";
  if (/-/.test(s) || /[\u2010-\u2015]/.test(s)) return "Password cannot contain hyphens or dashes.";
  if (s.length < 8) return "Password must be at least 8 characters.";
  if (!/[A-Z]/.test(s)) return "Include at least one capital letter.";
  if (!/[a-z]/.test(s)) return "Include at least one lowercase letter.";
  if (!/[0-9]/.test(s)) return "Include at least one number.";
  if (!/[!@#$%^&*()_+=\[\]{}|;:'",.<>?/`~\\]/.test(s)) return "Include at least one symbol (e.g. ! @ # $).";
  return null;
}
...
function displayNameFromAuthUser(user) {
  ...
  const raw =
    ...
    "writer";
  let name = sanitizeUsername(String(raw));
  if (!name) name = "writer";
...
async function ensureProfileForAuthUser(user) {
  ...
  if (p && p.id === user.id) {
    ...
    if (!p.authEmail && user.email) {
      p = { ...p, authEmail: user.email };
      await setJSON("writer-profile", p, false);
    }
    return p;
  }
  // New account on this device — start a profile keyed by auth user id
  let name = displayNameFromAuthUser(user);
  if (await isNameTaken(name)) {
    name = (name + Math.random().toString(36).slice(2, 5)).slice(0, 24);
  }
  const profile = {
    id: user.id,
    name,
...
async function signOutAuth() {
  ...
  try {
    await setJSON("writer-profile", null, false);
    localStorage.removeItem("writer-profile");
  } catch (_) {}
}
...
function ProfileGate({ onDone }) {
  ...
  useEffect(() => {
    (async () => {
      const sb = await getSupabase();
      if (!sb) {
        setError("Supabase is not configured. Add VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY, then redeploy.");
        return;
      }
      const { data } = await sb.auth.getSession();
      if (data?.session?.user) {
        const p = await ensureProfileForAuthUser(data.session.user);
        if (p) onDone(p);
      }
    })();
  }, [onDone]);
  ...
  const finishWithUser = async (user) => {
    const p = await ensureProfileForAuthUser(user);
    if (p) onDone(p);
  };
  ...
  const submitEmail = async () => {
    ...
    const sb = await getSupabase();
    if (!sb) {
      setError("Cloud login is not available (missing Supabase config).");
      return;
    }

    if (mode === "reset") {
      setSaving(true);
      const { error: err } = await sb.auth.resetPasswordForEmail(em, { redirectTo: redirectTo() });
      setSaving(false);
      if (err) {
    ...
    if (mode === "signup") {
      ...
      if (data?.session?.user) {
        await finishWithUser(data.session.user);
        return;
      }
    ...
    if (data?.user) await finishWithUser(data.user);
  };
      ...
      <div className="chn-fold chn-rise max-w-sm w-full p-8 rounded-2xl">
        ...
        <p className="text-sm mb-5" style={{ color: "var(--ink-soft)" }}>
          {mode === "reset"
            ...
            : "Use your email and password, or continue with Google. Writer name can be set after you sign in."}
        </p>
        ...
        {mode !== "reset" && (
          <div style={{ textAlign: "center", fontSize: 12, color: "var(--ink-soft)", marginBottom: 12 }}>or email</div>
        )}
        ...
        <input
          ...
          className="w-full px-3 py-2 mb-3 rounded-2xl outline-none"
          style={{
            background: "var(--paper-2)",
            border: "1px solid var(--border)",
        ...
        {mode !== "reset" && (
          <>
            ...
            />
            <p style={{ fontSize: 11, color: "var(--ink-soft)", margin: "0 0 12px", lineHeight: 1.4 }}>
              8+ characters, capital, lowercase, number, and symbol. No spaces or hyphens.
            </p>
          </>
        )}
        ...
        {mode === "signup" && (
          <>
            <label style={{ fontSize: 12, fontWeight: 600, color: "var(--ink-soft)" }}>Confirm password</label>
            <input
              type="password"
        ...
        <div style={{ marginTop: 16, fontSize: 13, textAlign: "center", color: "var(--ink-soft)" }}>
          {mode === "signin" && (
            <>
              <button type="button" onClick={() => { setMode("signup"); setError(""); setInfo(""); }} style={{ background: "none", border: "none", color: "#1D9BF0", cursor: "pointer", fontWeight: 600 }}>
                Create account
              </button>
              {" · "}
              <button type="button" onClick={() => { setMode("reset"); setError(""); setInfo(""); }} style={{ background: "none", border: "none", color: "#1D9BF0", cursor: "pointer", fontWeight: 600 }}>
                Forgot password
              </button>
            </>
          )}
          {mode === "signup" && (
            <button type="button" onClick={() => { setMode("signin"); setError(""); setInfo(""); }} style={{ background: "none", border: "none", color: "#1D9BF0", cursor: "pointer", fontWeight: 600 }}>
...
function Lobby({ profile, onOpenChain, onUpdateProfile, onOpenArchives, onOpenHelp, onLogout }) {
  ...
  const createChain = async ({ title, genre, theme, solo, target }) => {
    ...
    const chain = {
      ...
      likes: 0,
      watches: 0,
      storyTheme: storyThemeFromProfile(profile),
    };
...
function EditNameModal({ profile, onClose, onUpdateProfile, onLogout }) {
    ...
    >
      ...
      >
        ...
        <button
          type="button"
          onClick={async () => {
            if (!window.confirm("Sign out of Chain Stories on this device?")) return;
            await signOutAuth();
...
function ChainRoom({ profile, chainId, onBack, onUpdateProfile, onOpenHelp }) {
  ...
  const [themeMode, setThemeMode] = useState(() => loadThemeMode(profile?.id));
  const [autoState, setAutoState] = useState(() => loadAutoState(chainId, profile?.id));
  const [typingFocused, setTypingFocused] = useState(false);
  const [pendingThemeSwitch, setPendingThemeSwitch] = useState(false);
  const [quietThemeHint, setQuietThemeHint] = useState("");
  useEffect(() => {
    softApplyTheme(resolveTheme(profile, chain, {
      mode: themeMode,
      useStory: themeMode === "auto" && autoState.useStory,
    }));
  }, [profile?.themeId, profile?.themeCustom, chain?.storyTheme?.themeId, chain?.storyTheme?.themeCustom, themeMode, autoState.useStory]);
  useEffect(() => {
    if (themeMode !== "auto" || !chain) { setQuietThemeHint(""); setPendingThemeSwitch(false); return undefined; }
    const tick = () => {
      const st = loadAutoState(chainId, profile?.id);
      const elapsed = Date.now() - st.startedAt;
      const left = Math.max(0, THEME_AUTO_MS - (elapsed % THEME_AUTO_MS));
      const shouldStory = Math.floor(elapsed / THEME_AUTO_MS) % 2 === 1;
      if (shouldStory !== st.useStory) {
        if (typingFocused) {
          setPendingThemeSwitch(true);
          setQuietThemeHint(shouldStory ? "Story theme ready — applies when you leave the line box" : "Your theme ready — applies when you leave the line box");
        } else {
          const next = { startedAt: st.startedAt, useStory: shouldStory };
          saveAutoState(chainId, profile?.id, next);
          setAutoState(next);
          setPendingThemeSwitch(false);
          setQuietThemeHint(shouldStory ? "Switched to story theme" : "Switched to your theme");
          setTimeout(() => setQuietThemeHint(""), 4000);
        }
      } else if (!pendingThemeSwitch) {
        const mins = Math.max(1, Math.ceil(left / 60000));
        setQuietThemeHint(st.useStory ? `Story theme · your theme in ~${mins}m` : `Your theme · story theme in ~${mins}m`);
      }
    };
    tick();
    const id = setInterval(tick, 1000);
    return () => clearInterval(id);
  }, [themeMode, chainId, profile?.id, chain, typingFocused, pendingThemeSwitch]);
  useEffect(() => {
    if (!pendingThemeSwitch || typingFocused || themeMode !== "auto") return;
    const st = loadAutoState(chainId, profile?.id);
    const shouldStory = Math.floor((Date.now() - st.startedAt) / THEME_AUTO_MS) % 2 === 1;
    const next = { startedAt: st.startedAt, useStory: shouldStory };
    saveAutoState(chainId, profile?.id, next);
    setAutoState(next);
    setPendingThemeSwitch(false);
    setQuietThemeHint(shouldStory ? "Switched to story theme" : "Switched to your theme");
    setTimeout(() => setQuietThemeHint(""), 4000);
  }, [typingFocused, pendingThemeSwitch, themeMode, chainId, profile?.id]);
  const setThemeModeAndSave = (mode) => {
    setThemeMode(mode);
    saveThemeMode(profile?.id, mode);
    if (mode === "auto") {
      const fresh = { startedAt: Date.now(), useStory: false };
      saveAutoState(chainId, profile?.id, fresh);
      setAutoState(fresh);
    }
  };
      ...
      <div className="chn-shell">
        ...
        >
          ...
          <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: 8, marginTop: 10 }}>
            <span style={{ fontSize: 11, fontWeight: 600, color: "var(--ink-soft)" }}>THEME</span>
            <button type="button" onClick={() => setThemeModeAndSave("mine")}
              style={{ fontSize: 12, padding: "6px 12px", borderRadius: 999,
                border: themeMode === "mine" ? "1px solid var(--ink)" : "1px solid var(--border)",
                background: themeMode === "mine" ? "var(--btn)" : "var(--paper-card)",
                color: themeMode === "mine" ? "var(--btn-ink)" : "var(--ink)", cursor: "pointer", fontWeight: 600 }}>
              My theme
            </button>
            <button type="button" onClick={() => setThemeModeAndSave("auto")}
              style={{ fontSize: 12, padding: "6px 12px", borderRadius: 999,
                border: themeMode === "auto" ? "1px solid var(--ink)" : "1px solid var(--border)",
                background: themeMode === "auto" ? "var(--btn)" : "var(--paper-card)",
                color: themeMode === "auto" ? "var(--btn-ink)" : "var(--ink)", cursor: "pointer", fontWeight: 600 }}>
              Auto
            </button>
          </div>
          {quietThemeHint ? (
            <p style={{ fontSize: 12, color: pendingThemeSwitch ? "var(--danger)" : "var(--ink-soft)", margin: "6px 0 0 0" }}>
              {quietThemeHint}
            </p>
          ) : null}
        </div>
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Lock: code + copy + one padlock only — no labels */}
        <div
          style={{
            marginBottom: 20,
            padding: 12,
        ...
        >
          ...
          {canLock ? (
            <button
              ...
              title={chain.locked ? "Unlock" : "Lock"}
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                <button
                  ...
                  onClick={async () => {
                    ...
                    setTitleMsg(res?.ok ? "Title updated." : res?.message || "Could not update.");
                  }}
                  className="chn-btn chn-btn-primary"
                  style={{
                    padding: "8px 14px",
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    const res = await onApplyTitle?.(suggestion, { fromAI: true });
                    setTitleMsg(res?.ok ? `AI suggested: ${suggestion}` : res?.message || "Could not apply.");
                  }}
...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      try {
        await migrateLocalSharedToSupabase();
      } catch (_) {}
      try {
        const sb = await getSupabase();
        if (sb) {
          ...
          if (data?.session?.user) {
            ...
            if (p) setProfile(p);
            setLoading(false);
            // Keep session in sync (e.g. Google redirect, password recovery)
            sb.auth.onAuthStateChange(async (event, session) => {
              ...
              if (session?.user) {
                const next = await ensureProfileForAuthUser(session.user);
      ...
      // No auth session — require email/Google login (ignore legacy local-only profile)
      setProfile(null);
      setLoading(false);
    })();
  }, []);
