...
const HELP_SECTIONS = [
  ...
  },
  {
    ...
    body: "Set your personal theme in profile (White, Black, Sand, or Premium custom hex). Inside a chain: My theme keeps your colors; Auto alternates your theme and the story theme every 30 minutes with a soft fade. Story theme is stored on the chain (starter colors when the chain was created, or Set story theme by the starter). Auto will not hard-switch while you are typing a line — it waits until you leave the box and shows a quiet hint. Lobby always uses your personal theme.",
  },
  {
...
const BOT_EXTRA = [
  {
    ...
    answer: "In a chain use My theme (your colors) or Auto · 30 min (soft-fades between your theme and the story theme every 30 minutes). Story theme is saved on the chain by the starter. Auto waits if you are typing a line.",
  },
  {
...
function ChainRoom({ profile, chainId, onBack, onUpdateProfile, onOpenHelp }) {
  ...
  const pushStoryThemeFromMe = async () => {
    if (!chain || profile?.id !== chain.createdById) return;
    const latest = (await getJSON(`chain:${chainId}`, true)) || chain;
    const updated = { ...latest, storyTheme: storyThemeFromProfile(profile), updatedAt: Date.now() };
    await setJSON(`chain:${chainId}`, updated, true);
    setChain(updated);
    setQuietThemeHint("Story theme updated from your colors");
    setTimeout(() => setQuietThemeHint(""), 3000);
  };
      ...
      <div className="chn-shell">
        ...
        >
          ...
          <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: 8, marginTop: 10 }}>
            ...
            {isStoryStarter(chain, profile.id) && (
              <button type="button" onClick={pushStoryThemeFromMe}
                style={{ fontSize: 12, padding: "6px 12px", borderRadius: 999, border: "1px solid var(--border)",
